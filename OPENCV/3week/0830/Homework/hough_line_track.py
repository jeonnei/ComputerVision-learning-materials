...
def onChange(x):
    ...
    cv2.imshow('Hough Lines', np.hstack((img, img2)))

# 'Hough Lines'라는 이름의 윈도우 생성
cv2.namedWindow('Hough Lines')

# 트랙바 생성
cv2.createTrackbar('Threshold', 'Hough Lines', 100, 200, onChange)
cv2.createTrackbar('Min Line Length', 'Hough Lines', 50, 200, onChange)
cv2.createTrackbar('Max Line Gap', 'Hough Lines', 10, 100, onChange)
